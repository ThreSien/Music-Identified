...
//        Integer[] temparr= isPeak.toArray(new Integer[isPeak.size()]);
    ...
    public void peak_TransferScale(LinkedList isPeak) {
        if (!isPeak.isEmpty()) {//率先判斷isPeak是否為空
            ...
            for (String arr[] : sanskritArr) {
                ...
                for (int i = 0; i < arr.length - 1; i++) {
                    ...
                    if (PeakCount > arr.length / 3) { //若15個中有10個就算是該音階
        ...
        System.out.println("storescaleTem:" + storescaleTem.toString());

        String strScale[] = {"null", "DO", "DR", "RE", "RM", "MI", "FA", "FS", "SO", "SL", "LA", "LS", "SI"};//多寫一個0~12
        if (storescaleTem.size() >= 10) {//每10個取樣，判斷該區間誰是最多的音階，然後放入正式的LinkedList中
            ...
            if (!storeScale.isEmpty()) {//若正式的LinkedList為空，就先插入第一筆；反之，判斷是否與前一個輸出相同，不同再輸出
                if (!storeScale.peekLast().equals(strScale[maxIndex])) {
                    storeScale.offer(strScale[maxIndex]);
                }
            } else {
                storeScale.offer(strScale[maxIndex]);
            }
        }
        System.out.println("Scale:" + storeScale.toString());
    }
    
    //等待音樂輸入完畢->先判斷isPeak沒有更新->將所有isPeak轉為二維->進行公共子序列判斷(錯誤)
    ...
    public void voice_Compare(LinkedList userInput) {
        voice_Take();//取得一筆資料庫資料來與user輸入比對
    }
