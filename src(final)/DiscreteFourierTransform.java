...
import static cclo.Main.resultPage;
...
public class DiscreteFourierTransform extends BaseDataProcessor implements Share {
    ...
    public Data getData() throws DataProcessingException {
        ...
        if ((input != null) && (input instanceof DoubleData)) {
            ...
            voice_Process(cc);
            if (pMain.binaryControl == 0 && pMain.finishRec && RecBegin) {
...
//                userInput.clear();
            ...
            pMain.homePage.updateSpec(featureFreq);//改變頻譜
            //----------------------------------------------------------------------
            getPeak(featureFreq);
        } catch (Exception e) {
...
//        System.out.println("Peak:" + isPeak.toString());
        ...
        if (userInput != null && pMain.binaryControl == 0) {//使用者
            soundCnt++;
            if (pMain.startRec && (RecBegin || soundCnt > 5)) {//有出現5次有聲音才開始將資料輸入
...
//            System.out.println("上面dbInputStr:" + dbInputStr); //測試用
    ...
    public int[] voice_Take_One_DB() {
        if (allScale.isEmpty()) { //若allDBdata為空
            allScale.offer(pMain.resultPage.scale); //提取資料庫全部資料
            allSongName.offer(pMain.resultPage.songname);
        }
        if (pMain.finishRec == true) { //若使用者結束錄製
            String dbInputStr = allScale.peek();
            int DBwaitForCompare[] = get_TransferData(1, dbInputStr); //轉換一筆資料庫的資料供比對
...
//            System.out.println("userInputStr:" + userInput); //測試用
            ...
            //利用這裡先將資料處裡成依照原段落特徵減去段落平均
            int UserSum = 0;
            int UserCount = 0;
            for (int x : USERwaitForCompare) {
                if (x != -1) {
                    UserSum += x;
                    UserCount++;
                }
            }
            int UserMean = UserSum / UserCount;
            for (int i = 0; i < USERwaitForCompare.length; i++) {//將原本的值調整為 減去平均
                if (USERwaitForCompare[i] != -1) {
                    USERwaitForCompare[i] = USERwaitForCompare[i] - UserMean;
                }
            }
            //------------------------------------------------------------------
    ...
    public int[] voice_Take_User_One() {
        String userInputStr = "";
        if (pMain.finishRec == true) {//若使用者結束錄製
            userInputStr = userInput.toString();
//            System.out.println("userInputStr:" + userInput); //測試用
...
//        System.out.println("sub:"+sub); //測試用
    ...
    LinkedList<most_Similar> searchResultSort = new LinkedList<>();
...
//                System.out.println("dataInput：" + allScale.peek().length());
                ...
                do { //滑軌判斷，為了確保最後一次也要執行才能進入迴圈判斷
                    ...
                    //利用這裡先將資料處裡成依照原段落特徵減去段落平均
                    int DBSum = 0;
                    int DBCount = 0;//計算特徵中有聲音的數量
                    for (int x : DBwaitForCompareSub) {
                        if (x != -1) {
                            DBSum += x;
                            DBCount++;
                        }
                    }
                    int DBMean = DBSum / DBwaitForCompareSub.length;
                    for (int i = 0; i < DBwaitForCompareSub.length; i++) {//將原本的值調整為減去平均
                        if (DBwaitForCompareSub[i] != -1) {
                            DBwaitForCompareSub[i] = DBwaitForCompareSub[i] - DBMean;
                        }
                    }
                    //----------------------------------------------------------
                    ...
                    double similar = ((double) LCS.size() / (double) (USERwaitForCompare.length - 1)) * 100;//相似度運算方法
                    ...
                    slideNum += (int) (slideSize * 0.3); //排除先前比對的一半
            ...
            Collections.sort(searchResult,
                    new Comparator<most_Similar>() {
                public int compare(most_Similar o1, most_Similar o2) {
                    return o2.lcsLength - o1.lcsLength;
                }
            });
            for (most_Similar x : searchResult) {
                x.getAll();
                if (x.similar >= 45.0) {//列出相似度45%以上的資料(由高到低)
    ...
    boolean searchTimeReturn=false;
    public void voice_Compare_One() {
        int[] USERwaitForCompare, DBwaitForCompare;
        if (voice_Take_User_One() == null) {
            JOptionPane.showMessageDialog(null, "使用者輸入失敗");
        } else if (voice_Take_One_DB() == null) {
            JOptionPane.showMessageDialog(null, "取得資料庫失敗");
        } else {
            while (allScale.size() > 0) {
//                System.out.println("userInput：" + userInput.size());
//                System.out.println("dataInput：" + allScale.peek().length());
                USERwaitForCompare = voice_Take_User_One();
                DBwaitForCompare = voice_Take_One_DB(); //此函示只會取第一筆資料庫資料來比對
                int slideNum = 0;//儲存目前的位置
                int slideSize = (int) (USERwaitForCompare.length * 1.1); //去與本身1.1倍的特徵來比對
                most_Similar mostSimilar = new most_Similar();
                do { //滑軌判斷，為了確保最後一次也要執行才能進入迴圈判斷
                    ...
                    if (LCS.size() > mostSimilar.lcsLength) { //若找到比儲存lcs長度還大的，就換儲存當前資料
                        ...
                        mostSimilar.lcsLength = LCS.size();
                        mostSimilar.similar = similar;
                        mostSimilar.songName = SearchResult.songname;
                    }
                    LCS.clear(); //清除LCS內的暫存資料
...
//            SearchResult.resultStr = new String[searchResult.size()]; //因為不需更改RESULT
            searchResult.peek().getAll();
            SearchResult.searchTime = (int) searchResult.peek().simliarStar / 100;
            searchTimeReturn=true;
        }
    }
    ...
    class most_Similar {
        ...
        public void getAll() {
            System.out.println("songName：" + songName + "similar：" + similar + "% simliarStar：" + (double) simliarStar / 100 + "s lcsLength：" + lcsLength);
        }
    }
